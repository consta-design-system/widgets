...
Структуру компонентов можно псомтреть [тут](/?path=/docs/сommon-file-structure-components--page).
...
Желательно выносчить декомпоизцию в тело функции

## Разбивка компонента

Если вы собираетесь разбить компонент на более мелкие сущности, то нужно придерживаться следующих правил:

1. Он (компонент) должен [лежать в папке](/?path=/docs/сommon-file-structure-components--page) с дочерними компонентами (не быть в том же файле),
2. Должен быть оформлен как отдельный БЭМ блок.
...
Если вы хотите сохранить возвращаемое значение в переменную, то нужно переменную называть так же, как название метода, а импортируемому методу добавить приставку `get`:
