...
type MainElementProps = Omit<SVGAttributes<SVGGElement>, 'onClick' | 'onMouseOver'>
...
const renderPath = ({
  ...
  return (
    <path key={idx} d={renderArc(item) || undefined} fill={item.data.color} onClick={onClick} />
  )
}
  ...
  return (
    ...
    >
      {isEmptyPieArcDatum(data)
        ? renderPath({
            renderArc,
            item: { ...EMPTY_PIE_ARC_DATUM, ...getArcAnglesByHalfDonut(halfDonut) },
          })
        : data.map((item, idx) =>
            renderPath({
              renderArc,
