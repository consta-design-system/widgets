...
const getKnobs = (props: Partial<Props> = {}) => {
  return {
    ...
    arcLabelSize: select(
      'arcLabelSizes',
      arcLabelSizes,
      props.arcLabelSize ?? 's',
    ),
    formatValueForTooltip: boolean('formatValueForTooltip', true)
      ? cubeMeterFormatValue
      : String,
...
export const OneCircleWithLegend = createStory(
  () => (
    <DonutChart
      {...getKnobs({ data: donutOneCircleData, legendPosition: 'right' })}
    />
  ),
  {
...
export const OneCircleWithText = createStory(
  () => (
    <DonutChart
      {...getKnobs({ data: donutOneCircleData, value: '122', label: 'км' })}
    />
  ),
  {
...
export const OneCircleWithArcsLabels = createStory(
  () => (
    <DonutChart
      {...getKnobs({ data: donutOneCircleData, showArcLabels: true })}
    />
  ),
  {
