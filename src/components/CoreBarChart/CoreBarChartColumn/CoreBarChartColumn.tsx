...
import { styleOrientation } from './helpers';
...
export const getColumnCenter = (
  collection: HTMLCollection | undefined,
  isHorizontal: boolean,
) => {
  const children = collection || [];
  const { left, top } =
    children[isHorizontal ? 0 : children.length - 1].getBoundingClientRect();
...
export const CoreBarChartColumn: React.FC<Props> = ({
  ...
  const renderSection = (item: SectionItem | undefined, index: number) => {
    ...
    const isSectionLabel =
      isNumber(activeSectionIndex) && activeSectionIndex === index;
    const isActive =
      isSectionLabel ||
