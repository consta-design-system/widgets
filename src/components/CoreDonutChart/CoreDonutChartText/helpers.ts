...
export const getPaddingRatio =
  (condition: (halfDirection: HalfDonutDirection) => boolean) =>
...
export const getWidthPaddingRatio = getPaddingRatio(
  (halfDirection) => halfDirection === 'vertical',
);
...
export const getTextMaxSize = ({
  height,
  padding,
  ratio,
}: GetTextMaxSizeParams) => {
  return Math.round(
    (height - padding) * ratio * MAX_HEIGHT_AND_LINE_HEIGHT_DIFF_RATIO,
  );
};
