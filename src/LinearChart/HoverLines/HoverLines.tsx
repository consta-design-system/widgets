...
const HoverLine: React.FC<LineProps> = ({ position, className, value, onHover, onClick }) => {
        ...
        .map((lineValue, index) => {
          ...
          return (
            <React.Fragment key={index}>
              <HoverLine {...commonProps} className={cnHoverLines('Item', { isHoverable: true })} />
              <HoverLine {...commonProps} className={cnHoverLines('Item', { isActive })} />
            </React.Fragment>
          )
        })
        .filter(isDefined)}
